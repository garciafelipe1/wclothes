/**
 * Seed con force: borra carritos y productos existentes antes de recrear.
 * Uso: pnpm seed:force
 * Esto permite aplicar cambios en catalog.seed.ts (Color, Talle) sin hacer db:setup.
 */
import {
...
export default async function seedForceData({ container }: ExecArgs) {
  ...
  logger.info("SEED FORCE: borrando carritos, reservas y productos existentes...");

  const carts = await cartModuleService.listCarts({}, { take: 500 });
  if (carts.length > 0) {
    await cartModuleService.deleteCarts(carts.map((c: { id: string }) => c.id));
    logger.info("Borrados " + carts.length + " carrito(s)");
  }

  const reservationItems = await inventoryModuleService.listReservationItems(
    {},
    { take: 1000 }
  );
  if (reservationItems.length > 0) {
    await inventoryModuleService.deleteReservationItems(
      reservationItems.map((r: { id: string }) => r.id)
    );
    logger.info("Borradas " + reservationItems.length + " reserva(s) de inventario");
  }
  ...
  if (productIds.length > 0) {
    try {
      ...
      logger.info("Borrados " + productIds.length + " producto(s)");
    } catch (e) {
      logger.error("Error al borrar productos: " + (e instanceof Error ? e.message : String(e)));
      throw e;
    }
  }

  logger.info("Listo. Ejecutando seed normal...");
  ...
  logger.info("Publishable API key: usando la que tenés en .env");
  ...
  logger.info("Seed force finished. Productos recreados con Color y Talle.");
}
