...
**Si GET /app devuelve 200 pero el panel no carga o sale "Uncaught SyntaxError: Unexpected token '<'":** el navegador está recibiendo **HTML** (index.html) donde debería recibir **JavaScript** (p. ej. al cargar el script o un chunk). Eso pasa cuando se sirven las **fuentes de desarrollo** (`.medusa/client`) en vez del **bundle compilado**. El Dockerfile intenta: 1) `.medusa/server/public/admin` (build normal), 2) `.medusa/admin` (tras `medusa build --admin-only`), 3) `.medusa/client` (solo fallback). Revisar en los **logs del build** en Railway qué mensaje sale: "Admin: .medusa/server/public/admin", ".medusa/admin" o "fallback .medusa/client". Si sale el fallback, el panel no funcionará bien. En DevTools → Network, el script principal debe ser `application/javascript` y de tamaño grande (KB); si es ~424 bytes o type `text/html`, se está sirviendo el entry de desarrollo o el HTML.
